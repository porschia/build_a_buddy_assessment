class PurchaseOrder < ApplicationRecord
    ...
    def similarly_purchased_item purchased_item
        # first we have to determine if this is a stuffed_animal or if this
        # is an accessory
        other_line_items = []
        if purchased_item.class.name == "Stuffed Animal"
            other_line_items = LineItem.where(stuffed_animal_id: purchased_item.id)
        else
            other_line_items = LineItem.where(accessory: purchased_item.id)
        end

        # extract an array of similar items
        similar_items = self.other_products_purchased purchased_item other_line_items
    ...
    def other_products_purchased purchased_item, other_line_items
        ...
        other_purchase_order_ids = other_line_items.collect{|line_item| line_item.purchase_order_id if line_item.purchase_order_id != self.id}.flatten.uniq
